...
public class MyWebSocketHandler extends TextWebSocketHandler {
	...
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		...
		users.put(empNo, session);// 저장소에 사용자 식별 번호와 웹소켓 정보 담기
	}
	...
	protected void handleTextMessage(WebSocketSession session, TextMessage message) throws Exception {
		...
		String send = jsonObject.get("send").getAsString();	//송신자
		String receive = jsonObject.get("receive").getAsString(); //수신자
		...
		if (jsonObject.get("msg")==null) {	//메시지 확인인 경우
			try {
				messengerService.readMessage(mv);
			} catch (Exception e) {
				return;
			}
		} else {	//메시지 전송인 경우
